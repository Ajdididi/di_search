...
for i in range(0, len(kekka)):
    ...
    if not kekka[i][22] is None:
        print(f'   上限設定の種別：{kekka[i][22]}')
        print(f'   設定理由：{kekka[i][23]}')
        print(f'   1回量上限設定値：{kekka[i][24]}')
        print(f'   1回量上限設定値（第一単位）：{kekka[i][25]} {kekka[i][26]}')
    else:
        print('   該当しない')         
    print('【自動車運転等の注意喚起】')
    ...
    if not kekka[i][31] is None:
        ...
        print('   該当しない')
    print('【インラインフィルター】')
    if not kekka[i][38] is None:
        print(f'   適用上の注意（添付文書）：{kekka[i][39]}')
        print(f'   対応：{kekka[i][40]}')
    else:
        print('   該当しない')    
    print('【冷所医薬品の室温での安定性】')
    ...
    if not kekka[i][45] is None:    #ここだけ試しに該当すれば表示するスタイルにしてみる
        print('【AddDis設定】')
        print(f'   比重：{kekka[i][45]}')
    if not kekka[i][46] is None:
        print('【AddDis設定】')
        print(f'   粉の重量：{kekka[i][46]}')
